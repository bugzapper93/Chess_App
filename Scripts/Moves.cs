...
namespace Chess_App
{
    ...
    public struct Moveset
    {
        ...
        public List<Position> defended;
    }
    class Moves
    {
        public static Moveset Get_All_Piece_Moves(Chessboard board, int i, int j)
        {
            List<Move> moves = new List<Move>();
            List<Pin> pins = new List<Pin>();
            List<Position> defended = new List<Position>();

            Position white_king_position = new Position { row = -1, column = -1 };
            Position black_king_position = new Position { row = -1, column = -1 };

            for (int a = 0; a < Variables.Board_Size; a++)
                for (int b = 0; b < Variables.Board_Size; b++)
                {
                    if (board.pieces[a, b].value == (Pieces.King | Pieces.White))
                        white_king_position = new Position { row = a, column = b };
                    if (board.pieces[a, b].value == (Pieces.King | Pieces.Black))
                        black_king_position = new Position { row = a, column = b };
                }
            ...
            Position king_pos = (color & 24) == Pieces.White ? white_king_position : black_king_position;

            // Getting piece pins
            List<Pin> piece_pins = new List<Pin>();
            if (board.possible_moves_pins.pins != null)
                foreach (Pin pin in board.possible_moves_pins.pins)
                {
                    if (Compare_Positions(start, pin.pinned))
                        piece_pins.Add(pin);
                }
            ...
            switch (base_piece)
            {
                case Pieces.Pawn:
                    ...
                    {
                        if (!Is_Legal_For_All_Pins(start, considered_position, king_pos, piece_pins))
                            correct_move = false;
                    }
                    if (Pieces.Get_Value(board.pieces, considered_position) == 0 && i + row_diff >= 0 && i + row_diff < 8 && correct_move)
                    {
                        if (Pieces.Check_Friendly_Pos(board.pieces, start, considered_position) && correct_move)
                        {
                            moves.Add(new Move
                            {
                                piece = temp_piece,
                    ...
                    else
                    {
                        ...
                        {
                            defended.Add(considered_position);
                        }
                    }
                    ...
                    if ((color == Pieces.White && i == 6) || (color == Pieces.Black && i == 1))
                    {
                        ...
                        if (Pieces.Get_Value(board.pieces, considered_position) == 0 && Pieces.Check_Move(board.pieces, start, considered_position) && correct_move)
                        {
                            if (Pieces.Check_Friendly_Pos(board.pieces, start, considered_position)) 
                            { 
                                moves.Add(new Move
                                {
                                    piece = temp_piece,
                    ...
                    foreach (int var in vars)
                    {
                        ...
                        if (Pieces.Check_Move(board.pieces, start, considered_position) && Pieces.Get_Value(board.pieces, considered_position) != 0 && correct_move)
                            if (Pieces.Check_Friendly_Pos(board.pieces, start, considered_position))
                            {
                                moves.Add(new Move
                                {
                                ...
                                while (true)
                                {
                                    ...
                                    else
                                    {
                                        if (((next_value & 7) == Pieces.King) && ((next_value & 24) != (temp_piece & 24)))
                                        {
                                            Pin pin = new Pin
                                            {
                                                start = new Position { row = row_diff, column = col_diff },
                                                pinned = considered_position
                                            };
                                            pins.Add(pin);
                                        }
                                        break;
            ...
            return new Moveset { moves = moves, pins = pins, defended = defended };
        }
        
        public static Moveset Get_All_Possible_Moves(Chessboard board, bool king_only = false)
        {
            List<Move> moves = new List<Move>();
            List<Pin> pins = new List<Pin>();
            List<Position> defended = new List<Position>();

            int current_color = board.is_white_turn ? Pieces.White : Pieces.Black;
            bool finished = false;
            ...
            for (int i = 0; i < Variables.Board_Size; i++)
            {
                ...
                {
                    if (board.pieces[i, j].value != 0 && (board.pieces[i, j].value & 24) == current_color)
                    {
                        if (king_only && (board.pieces[i, j].value & 7) == Pieces.King)
                        {
                            Moveset moveset = Get_All_Piece_Moves(board, i, j);
                            moves.AddRange(moveset.moves);
                            pins.AddRange(moveset.pins);
                            defended.AddRange(moveset.defended);
                            finished = true;
                            break;
                        }
                        else
        ...
        //        for (int j = 0; j < Variables.Board_Size; j++)
        //        {
        //            if (board.pieces[i, j].value == (Pieces.King | Pieces.White))
        //                white_king_position = new Position { row = i, column = j };
        //            if (board.pieces[i, j].value == (Pieces.King | Pieces.Black))
        //                black_king_position = new Position { row = i, column = j };
        //        }
        ...
        private static bool Is_Along_Pin(Position start, Position end, Position king_pos, Pin pin)
        {
            int move_row = end.row - start.row;
            int move_col = end.column - start.column;

            return (move_row * pin.start.column == move_col * pin.start.row);
        }
        private static bool Is_Legal_For_All_Pins(Position start, Position end, Position king_pos, List<Pin> piece_pins)
        {
            foreach (Pin pin in piece_pins)
            {
                if (!Is_Along_Pin(start, end, king_pos, pin))
                    return false;
            }
            return true;
        }
